...
def hashing(mode:HashingMode, text=""):
    ...
    if mode == HashingMode.DECODE:
        ...
        for graph in graphs:
            if graph != "" and graph != "%":
                key = find_key_by_value(words, "%" + str(graph) + "%")
                if key is not None:
                    new_text += key
        return new_text