...
func _ready() -> void:
	...
	Updatedate.request("/auth/get?name=message")
	Updatedate.request_completed.connect(func(data, url):
		if url == "/auth/get?name=message":
			if data and data.has("nums"):
				Updatedate.save("server_message", data.nums[0], false)
				create_messages(data.nums[0])
				)
	create_messages(Updatedate.load_game("server_message", []))
	await get_tree().create_timer(0.1).timeout
	show()
func create_messages(data):
	for child in $ScrollContainer/VBoxContainer.get_children():
		if child.name != "instance":
			child.queue_free()
	for message in data:
		if message.has("type"):
			var box = $ScrollContainer/VBoxContainer/instance.duplicate()
			...
			if message.type == "guid" or message.type == "update":
				...
				box.get_node("HBoxContainer/MarginContainer/VBoxContainer/HBoxContainer/Button2").pressed.connect(func ():
					...
					if d:
						...
						Notification.add_notif("اتصال اینترنت خود را بررسی کنید!", Notification.ERROR))
