...
func _ready() -> void:
	...
	if data.has("ticket"):
		...
		if calendar:
			...
			var event = calendar.getCalendarEvents(int(Updatedate.load_game("start_ticket", "0")), ((int(date) / 1000) +  86400) * 1000)
...
func _on_button_pressed() -> void:
	if Updatedate.load_game("accept_account", false):
		...
		Notification.add_notif("حساب شما تأیید نشده! به پشتیبانی مراجعه کنید.", Notification.ERROR)
